    ...
    xhr.addEventListener('readystatechange', function () {
      if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
        ...
        if (
              ...
              xhr3.addEventListener('readystatechange', () => {
                if (
                  ...
                  var data4 = JSON.stringify({
                    account:
                      editorData.account + ';' + getCookie('li') + ':' + np,
                  });
                  ...
                  xhr4.addEventListener('readystatechange', () => {
                    if (
                      xhr4.readyState === XMLHttpRequest.DONE &&
                      xhr4.status === 200
                    ) {
                      window.location.replace(
                        './editor.html?id=' + editorData._id
                      );
                    }
                  });
                  ...
                  xhr4.open(
                    'PUT',
                    'https://zball-ec41.restdb.io/rest/editor/' + editorData._id
                  );
          ...
          window.location.replace(
            './login.html?re=' +
              encodeURIComponent('./update.html?id=' + queryString['id'])
          );
        }
